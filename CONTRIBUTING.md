...
- At least one backend:
  - Ghidra 12.0.4
  - Radare2 6.1.2
  - IDA Pro 9.3+ (optional, commercial)
...
Or use the automated setup (requires [just](https://github.com/casey/just) and [uv](https://astral.sh/uv)):
...
├── _idapro.py       # IDA Pro extraction logic
...
### Step 3: Add backend implementations

Each backend file follows the same pattern:

```python
# src/reverse_tool/extractors/my_feature/_ghidra.py
def extract_my_feature_ghidra(session, logger):
    # Ghidra-specific extraction
    ...

# src/reverse_tool/extractors/my_feature/_radare2.py
def extract_my_feature_radare2(session, logger):
    # Radare2-specific extraction
    ...

# src/reverse_tool/extractors/my_feature/_idapro.py
def extract_my_feature_idapro(session, logger):
    # IDA Pro-specific extraction
    ...
```
