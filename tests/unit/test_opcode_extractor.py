...
class TestOpcodeExtractor:
    ...
    def test_supported_backends_includes_idapro(self, opcode_extractor):
        assert "idapro" in opcode_extractor.supported_backends
    ...
    def test_extract_with_null_backend(self, opcode_extractor, tmp_path):
        ...
        first = result.data["opcodes"][0]
        assert first["mnemonic"] == "push"
        assert first["instruction"] == "push"
        assert "index" in first
        assert "size" in first
        assert "bytes" in first
        assert "section" in first

    def test_write_output_creates_jsonl_and_csv(self, opcode_extractor, tmp_path):
